...
class UpdateController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->has('size'))
        {
            $size = Size::where('id', $request->input('size'))->first();
    ...
    public function save(Request $request){
        ...
        if($request->has('products')){
            foreach($request->input("products") as $id => $status){
                $size->products()->sync(Product::where('id', $id)->first());
            }
        }

        foreach($size->columns()->get() as $col){
            $col->delete();
        }

        foreach($size->rows()->get() as $col){
            $col->delete();
        }
        ...
        return  redirect('/admin/sizes/')->with("success", "Size Chart " . $request->input("name") . " was saved successfully.");
    }
}