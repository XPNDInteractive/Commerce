...
@section('content')
    ...
    @if (count($errors->all()) !== 0 || session()->has('success'))
        <form style="height: calc(100% - 166px);" method="post" action="/admin/filters/update/save">
    @else
        <form style="height: calc(100% - 116px);" method="post" action="/admin/filters/update/save">
    @endif
        @csrf
        <input type="hidden" name="filter" value="{{$datasets['filters']['rows']->id}}"/>
            ...
            <div class="col-sm-12 col-md-8 p-0 h-100 overflow-auto scrollbox text-dark ">
                <div class="row m-0">
                    <div class="col-sm-12 col-md-6 bg-white border p-5">
                        <label>* Name</label>
                        <input class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" type="text" name="name"  value="{{old('name') == null ? $datasets['filters']['rows']->name:old('name')}}"/>
                        @if ($errors->has('name'))
                     ...
                     <div class="col-sm-12 col-md-6 bg-white border p-5">
                        ...
                        <input class="form-control" type="text" name="description"  value="{{old('description') == null ? $datasets['filters']['rows']->description:old('description')}}"/>
                ...
                <div class="row m-0">
                    ...
                    <div class="col-12 bg-white border p-5">
                            ...
                            <input name="active" type="checkbox" class="custom-control-input" id="active" {{$datasets['filters']['rows']->active == true ? 'checked':''}}>
            ...
            <div class="col-sm-12 col-md-4 p-0 h-100  scrollbox bg-light border-right">
                ...
                <ul class="nav tabs border-0 bg-white justify-content-start" id="myTab" role="tablist">
                    ...
                    <li class="nav-item col p-0">
                        <a class="nav-link rounded-0 border text-dark text-center active" id="home-tab" data-toggle="tab" href="#permissions" role="tab" aria-controls="home" aria-selected="true">Tags</a>
                    </li>

                     <li class="nav-item col p-0">
                        <a class="nav-link rounded-0 border text-dark text-center" id="home-tab" data-toggle="tab" href="#categories" role="tab" aria-controls="home" aria-selected="true">Categories</a>
                    </li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="permissions" role="tabpanel" aria-labelledby="profile-tab">
                        <div class="row m-0 p-3">
                            
                            <a href="/admin/filter/tag/create?filter={{$datasets['filters']['rows']->id}}" class="btn btn-primary btn-sm ml-auto">New</a>
                        </div>
                         <ul class="list-group rounded-0 border-0 select-list">
                              ...
                              @foreach($datasets['links']['rows'] as $row)
                                <li class="bg-light  p-0  list-group-item d-flex justify-content-start align-items-center">
                                    <a class="col p-3 text-muted" href="/admin/filter/tag/update?filter={{$datasets['filters']['rows']->id}}&tag={{$row->id}}">{{$row->name}}</a>
                                        ...
                                        <div class="modal-dialog rounded-0" role="document">
                                            <div class="modal-content rounded-0">
                                                <div class="modal-header bg-danger rounded-0">
                                                    <h5 class="modal-title text-white" id="exampleModalLongTitle">Delete Filter Tag {{$row->name}}</h5>
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <a class="btn btn-danger" href="/admin/filter/tag/delete?filter={{$datasets['filters']['rows']->id}}&tag={{$row->id}}">Delete</a>
                                                </div>
                                            </div>
                                        </div>
                                </div>
                    ...
                    <div class="tab-pane" id="categories" role="tabpanel" aria-labelledby="home-tab">
