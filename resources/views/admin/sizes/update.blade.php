...
@section('content')
    ...
    @endif
    @if (count($errors->all()) !== 0)
        <form style="height: calc(100% - 166px);" method="post" action="/admin/sizes/update/save">
    @else
        <form style="height: calc(100% - 116px);" method="post" action="/admin/sizes/update/save">
    @endif
        @csrf
         <input type="hidden" name="size" value="{{$datasets['size']['rows']->id}}"/>
            ...
            <div class="col-sm-12 col-md-8 p-0  scrollbox text-dark ">
                <div class="row m-0">
                    <div class="col-sm-12 col-md-12 bg-white border-bottom p-5">
                        <label>* Name</label>
                        <input class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" type="text" name="name"  value="{{$datasets['size']['rows']->name}}"/>
                        @if ($errors->has('name'))
                ...
                <div class="row m-0 p-5 border-bottom align-items-center">
                    <div class="col-6">
                        ...
                        <table style="table-layout:fixed;" class="table table-bordered size-table m-0 " >
                            ...
                            <thead>
                                <tr class="">
                                    ...
                                    @if($datasets['size']['rows']->columns()->count() > 0)  
                                        @foreach($datasets['size']['rows']->columns()->orderBy('id')->get() as $col)
                                           
                                            <th class="text-left">
                                                <div class="row m-0 align-items-center">
                                                    <input style="font-weight: 500;"value="{{$col->name}}" placeholder="column name" class=" border-0 form-control form-control-sm" type="text" name="col[]"/>
                                                </div>
                                            </th>
                                        @endforeach
                                    @else
                            ...
                            <tbody>
                                ...
                                <tr class="d-none row-clone">
                                 ...
                                 @if($datasets['size']['rows']->rows()->count() > 0)
                                       
                                    @for($i = 0; $i < $datasets['size']['rows']->rows()->distinct('size_row_id')->count('size_row_id'); $i++)
                                    
                                    
                                        <tr class="">
                                            
                                            <td class="text-left ">
                                                <label class="m-0 row-number">{{$i + 1}}</label>
                                            </td>

                                            @foreach($datasets['size']['rows']->columns()->orderBy('id')->get() as $k => $col)
                                            <td class="text-left column-clone bg-light">
                                                <div class="row m-0 align-items-center">
                                                    <input class="bg-light border-0 form-control form-control-sm row-input" value="{{$datasets['size']['rows']->rows()->where('size_row_id', $i)->where('size_column_id', $col->id)->first()->value}}" type="text" name="row[{{$i}}][{{$k}}]"/>
                ...
                <div class="row m-0">
                    <div class="col-12 bg-white  p-5">
                        <div class="custom-control custom-checkbox">
                            <input name="active" type="checkbox" class="custom-control-input" id="active" {{$datasets['size']['rows']->active == 1 ? ' checked':''}}>
                    ...
                    <p class="m-0 text-muted mb-2">Select what sizes this product will fall under.</p>
                        <ul class="list-group rounded-0 border-0 select-list">
                            ...
                            @foreach($datasets['products']['rows'] as $row)
                                ...
                                <li class="select-list-item border-left border-right select-list-item list-group-item d-flex justify-content-start align-items-center {{!is_null($datasets['size']['rows']->products()->where('products.id', $row->id)->first()) ? ' active':''}}">
                                    <input name="products[{{$row->id}}]" type="checkbox" class="col-1" {{!is_null($datasets['size']['rows']->products()->where('products.id', $row->id)->first()) ? ' checked':''}}/>
