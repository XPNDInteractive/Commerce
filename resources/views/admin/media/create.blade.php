...
@section('content')
    ...
    @endif
    @if (count($errors->all()) !== 0)
        <form style="height: calc(100% - 166px);" method="post" action="/admin/media/create/save" enctype="multipart/form-data">
    @else
        <form style="height: calc(100% - 116px);" method="post" action="/admin/media/create/save" enctype="multipart/form-data">
    @endif
        @csrf
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active p-3" id="permissions" role="tabpanel" aria-labelledby="home-tab">
                        <div style=" border: #eee 3px dashed; padding: 3rem; " class="placeholder row m-0 justify-content-center flex-column align-items-center">
                            <i style="font-size: 230px; color: #ccc;" class="fas fa-image"></i>
                            <img class="img w-100" src=""/>
                            <input name="media" id="myInput" type="file" style="visibility:hidden" onChange="readFile(this)"/>
                            <button type="button" class="btn btn-primary mt-3"  onclick="$('#myInput').click();">Upload Media</button>
                            <script>
