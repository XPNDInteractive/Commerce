...
@section('content')
    ...
    @endif
    @if (count($errors->all()) !== 0)
        <form style="height: calc(100% - 166px);" method="post" action="/admin/menu/item/update/save">
    @else
        <form style="height: calc(100% - 116px);" method="post" action="/admin/menu/item/update/save">
    @endif
        @csrf
            ...
            <div class="col-sm-12 col-md-8 p-0 h-100 overflow-auto scrollbox text-dark ">
                <div class="row m-0">
                    <div class="col-sm-12 col-md-12 bg-white border p-5">
                        <label>* Name</label>
                        <input class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" type="text" name="name"  value="{{old('name') == null ? $datasets['link']['rows']->name:old('name')}}"/>
                        @if ($errors->has('name'))
                 ...
                 <div class="row m-0">
                   <div class="col-sm-12 col-md-12 bg-white border p-5">
                        <label>* URL</label>
                        <input class="path form-control {{ $errors->has('path') ? ' is-invalid' : '' }}" type="text" name="path"  value="{{old('path') == null ? $datasets['link']['rows']->path:old('path')}}"/>
                        @if ($errors->has('path'))
                            <span class="invalid-feedback d-block" role="alert">
                ...
                <div class="row m-0">
                    <div class="col-sm-12 col-md-12 bg-white border p-5">
                        ...
                        <textarea class="form-control"  name="description">{{old('description') == null ? $datasets['link']['rows']->description:old('description')}}</textarea>
                   ...
                   <div class="col-sm-12 col-md-12 bg-white border p-5">
                        <label>Parent</label>
                         <select name="parent" class="custom-select rounded-0 {{ $errors->has('parent') ? ' is-invalid' : '' }}">
                            @if($datasets['link']['rows']->roles()->count() == 0)
                            ...
                            @foreach($datasets['links']['rows'] as $row)
                                @if($datasets['link']['rows']->parent_id == $row->id)
                ...
                <div class="row m-0">
                    ...
                    <div class="col-12 bg-white border p-5">
                            ...
                            <input name="active" type="checkbox" class="custom-control-input" id="active"  {{$datasets['link']['rows']->active == true ? 'checked':''}}>
                    ...
                    <div class="tab-pane active" id="permissions" role="tabpanel" aria-labelledby="home-tab">
                         ...
                         <ul class="list-group rounded-0 border-0 select-list ">
                              ...
                              @foreach($datasets['roles']['rows'] as $row)
                                    @if(!is_null($datasets['link']['rows']->roles()->where('roles.id', $row->id)->first()))
