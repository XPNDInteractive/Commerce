...
@section('content')
    ...
    @endif
    @if (count($errors->all()) !== 0)
        <form style="height: calc(100% - 166px);" method="post" action="/admin/reviews/update/save">
    @else
        <form style="height: calc(100% - 116px);" method="post" action="/admin/reviews/update/save">
    @endif
        @csrf
        <div class="row m-0 ">
           
            <input type="hidden" name="review" value="{{$datasets['review']['rows']->id}}"/>
            <div class="col-sm-12 col-md-8 p-0  scrollbox text-dark ">
                <div class="row m-0">
                    <div class="col-sm-12 col-md-12 bg-white border-bottom p-5">
                        ...
                        <select class="custom-select rounded-0 {{ $errors->has('user') ? ' is-invalid' : '' }}" name="user" >
                            ...
                            @foreach($datasets['users']['rows'] as $user)
                                ...
                                @if($user->name == $datasets['review']['rows']->user_id)
                    ...
                    <div class="col-sm-12 col-md-12 bg-white border-bottom p-5">
                        <label>Title</label>
                        <input class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" type="text" name="title"  value="{{$datasets['review']['rows']->title}}"/>
                        @if ($errors->has('title'))
                            <span class="invalid-feedback d-block" role="alert">
                     ...
                     <div class="col-sm-12 col-md-12 bg-white border-bottom p-5">
                        <label>Message</label>
                        <textarea class="form-control {{ $errors->has('message') ? ' is-invalid' : '' }}"  name="message">{{$datasets['review']['rows']->message}}</textarea>
            ...
            <div class="col-sm-12 col-md-4 p-0 scrollbox bg-light border-left">
                 <div class="bg-light mb-4 p-5 border-bottom">
                    ...
                    <div class="rating" data-rate-value={{$datasets['review']['rows']->rating}}></div>
                    <input class="rating-input" type = "hidden" name="rating" value="{{$datasets['review']['rows']->rating}}"/>
