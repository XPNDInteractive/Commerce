    ...
    <div class="row filters m-0">
        ...
        <div class="row m-0 col gradient-panel  p-3 align-items-center justify-content-center">
            ...
            <div class="dropdown show ml-auto">
                <a class="btn btn-success btn-sm" href="/admin/variants/create" role="button" aria-haspopup="true" aria-expanded="false">
            ...
            <tbody style="">
                  @foreach($datasets['variants']['rows'] as $row)
                    ...
                    <tr >
                        ...
                        <td>  @foreach($row->products()->get() as $perm)
                                    <span class="badge rounded-0 badge-primary d-block mt-1">{{$perm->name}}</span>
                                @endforeach
                        </td>
                         @foreach($datasets['attributes']['rows'] as $col)
                            @php 
                                $matched = false
                            @endphp
                            @foreach($row->attributeValues()->get() as $perm)
                                @if($perm->attributes()->first()->name == $col->name)
                        ...
                        <td >{{$row->upc}}</td>
                        <td >{{$row->sku}}</td>
                        <td >{{is_null($row->price) ? '': "$".$row->price}}</td>
                        <td >{{$row->quantity}}</td>
                        <td >{{$row->available_quantity}}</td>
                       
                        <td>
                            <img style="height: 30px; width: 30px;" src="{{base64_decode($row->media_id)}}" alt="...">
                        ...
                        <td >
                            <a href="/admin/variants/update?variant={{$row->id}}" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
