...
@section('content')
    ...
    @endif
    @if (count($errors->all()) !== 0)
        <form style="height: calc(100% - 166px);" method="post" action="/admin/filter/tag/update/save">
    @else
        <form style="height: calc(100% - 116px);" method="post" action="/admin/filter/tag/update/save">
    @endif
        @csrf
        <input type="hidden" name="filter" value="{{$request->input('filter')}}"/>
        <input type="hidden" name="id" value="{{$request->input('tag')}}"/>
            ...
            <div class="col-sm-12 col-md-12 p-0 h-100 overflow-auto scrollbox text-dark ">
                ...
                <div class="row m-0">
                    <div class="col-sm-12 col-md-12 bg-white border p-5">
                        ...
                        <textarea class="form-control {{ $errors->has('description') ? ' is-invalid' : '' }}"  name="description">{{old('description') == null ? $datasets['link']['rows']->description:old('description')}}</textarea>
